...
theorem exercise_4.14 {I : Type*} [topological_space I]
    [linear_order I] (f : I → I) (hf : continuous f) :
    ∃ (x : I), f x = x :=
sorry

theorem exercise_4.15 {f : ℝ → ℝ} 
...
theorem exercise_5.1 {α : Type*} [linear_ordered_field α] 
    {f : α → α} (hf : ∀ x y : α, abs (f x - f y) ≤ (x - y) ^ 2) :
    ∃ (c : α), f = function.const α c :=
sorry

theorem exercise_5.2 {α : Type*} [linear_order α] 
...
theorem exercise_5.7 {α : Type*} [linear_order α]
...
theorem exercise_5.15 {f : ℝ → ℝ} (hf : twice_differentiable_at ℝ f a) 
...
theorem exercise_6.1 {α : Type*} [linear_order α] 
...
theorem exercise_6.2 {α : Type*} [linear_ordered_field α]
