theorem exercise_2.12a {α : Type*} [linear_order α] {f : ℕ → α} 
    (hf : injective f) (l : filter α) (a : α) :
    tendsto (λ n, (f n)) l a ↔ tendsto f l a :=
sorry

theorem exercise_2.12b {α : Type*} [linear_order α] {f : ℕ → α} 
    (hf : surjective f) {l : filter α} {a : α} (h : tendsto (λ n, f n) at_top l) :
    tendsto (λ n, f (f.symm n)) at_top l :=
sorry

theorem exercise_2.26 {M : Type*} [topological_space M]
    (U : set M) : is_open U ↔ ∀ (x : M), x ∈ U → ¬ is_limit_point (set.compl U) x :=
sorry

theorem exercise_2.29 (M : Type*) [metric_space M] :
    function.bijective (λ (U : set M), closure U) :=
sorry

theorem exercise_2.32a (A : set ℕ) : is_clopen A :=
sorry

theorem exercise_2.41 (m : ℕ) (norm : ℝ^m → ℝ) :
  compact (set.ball (0 : ℝ^m) 1) :=
sorry

theorem exercise_2.46 {M : Type*} [metric_space M]
...
theorem exercise_2.57 {X : Type*} [topological_space X]
...
theorem exercise_3.1 {f : ℝ → ℝ} (hf : ∀ x y, abs (f x - f y) ≤ abs (x - y) ^ 2) :
  function.is_constant f :=
sorry

theorem exercise_3.4 (n : ℕ) :
  tendsto (λ n, (sqrt (n + 1) - sqrt n)) at_top (𝓝 0) :=
sorry

theorem exercise_3.11a {f : ℝ → ℝ} {a b : ℝ} (hf : ∀ x ∈ (a, b), deriv_two f x) :
  ∀ x ∈ (a, b), ∃ (l : ℝ), tendsto (λ h, (f (x - h) - 2 * f x + f (x + h)) / h ^ 2) (𝓝 0) (𝓝 l) :=
sorry

theorem exercise_3.17c-i (x : ℝ) : smooth_at ℝ ℝ (λ x, exp 2 * exp (1 - x) * exp (x + 1)) x :=
sorry

theorem exercise_3.17c-ii (x : ℝ) : 
    x ≤ -1 ∨ x ≥ 1 → bump_function x = 0 :=
sorry

theorem exercise_3.18 {L : set ℝ} (hL : is_closed L) :
    ∃ (f : ℝ → ℝ), is_smooth f ∧ ∀ (x : ℝ), f x = 0 ↔ x ∈ L :=
sorry

theorem exercise_3.43a {f : ℝ → ℝ} 
    (hf : riemann_integrable f) :
    riemann_integrable (f ∘ λ x, x * sin (1 / x)) :=
sorry

theorem exercise_3.53 {α : Type*} [linear_ordered_field α] 
    {f g : ℝ → α} (hf : integrable f) (hg : integrable g) :
    integrable (λ x, max (f x) (g x)) ∧ integrable (λ x, min (f x) (g x)) :=
sorry

theorem exercise_3.59 {α : Type*} [linear_ordered_semiring α]
    {a : ℕ → α} (ha : ∀ n, 0 ≤ a n) (h : series a) :
    series (λ n, sqrt (a n) / n) :=
sorry

theorem exercise_3.63 (p : ℕ) (h : p > 1) :
  series.converges (λ k, (1 : ℝ) / (k * (log k) ^ p)) :=
sorry

theorem exercise_4.15a {α : Type*} 
...
theorem exercise_6.43 {y : ℝ} : differentiable ℝ (λ x, integral (λ x, exp (-x) * sin (x + y)) x) :=
sorry

theorem exercise_6.49a {α : Type*} 
