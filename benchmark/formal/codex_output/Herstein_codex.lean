theorem exercise_2.1.18 {G : Type*} [group G] 
    (hG : fintype G) (hG2 : 2 ∣ fintype.card G) :
    ∃ (a : G), a ≠ 1 ∧ a = a⁻¹ :=
sorry

theorem exercise_2.1.21 (G : Type*) [group G] (hG : fintype.card G = 5) :
    abelian_group G :=
sorry

theorem exercise_2.1.26 {G : Type*} [group G] (a : G) 
    (hG : fintype G) : ∃ (n : ℕ), a ^ n = 1 :=
sorry

theorem exercise_2.1.27 {G : Type*} [group G] 
    (hG : fintype G) : ∃ (m : ℕ), ∀ (a : G), a ^ m = 1 :=
sorry

theorem exercise_2.2.3 {G : Type*} [group G] 
    (a b : G) (i j k : ℕ) (h : (a * b) ^ i = a ^ i * b ^ i) 
    (h' : (a * b) ^ j = a ^ j * b ^ j) (h'' : (a * b) ^ k = a ^ k * b ^ k) :
    is_abelian G :=
sorry

theorem exercise_2.2.5 {G : Type*} [group G] 
...
theorem exercise_2.3.17 {G : Type*} [group G] (a x : G) 
    (h : a ∈ comm_group.comm_of G) :
    conjugate x⁻¹ a x = x⁻¹ * conjugate a x :=
sorry

theorem exercise_2.3.19 {G : Type*} [group G] {M : subgroup G}
    (hM : ∀ (x : G), x⁻¹ * M * x ⊆ M) :
    ∀ (x : G), x⁻¹ * M * x = M :=
sorry

theorem exercise_2.3.16 {G : Type*} [group G] 
    (hG : ¬ ∃ (H : subgroup G), H ≠ ⊥ ∧ H ≠ ⊤) :
    ∃ (p : ℕ), nat.prime p ∧ G ≅ (multiplicative (fin p)) :=
sorry

theorem exercise_2.3.21 {G : Type*} [group G] 
    {A B : subgroup G} (h : ∀ b : G, b ∈ B → ∀ a : G, a ∈ A → b⁻¹ * a * b ∈ A) :
    is_subgroup (A * B) :=
sorry

theorem exercise_2.3.22 {G : Type*} [group G] 
...
theorem exercise_2.3.28 {G : Type*} [group G] 
    {M N : subgroup G} (hM : ∀ (x : G), x⁻¹ * M * x ⊆ M) 
    (hN : ∀ (x : G), x⁻¹ * N * x ⊆ N) :
    ∀ (x : G), x⁻¹ * (M * N) * x ⊆ M * N :=
sorry

theorem exercise_2.3.29 {G : Type*} [group G] {M : subgroup G} 
    (hM : ∀ (x : G), x⁻¹ * M * x ⊆ M) :
    ∀ (x : G), x⁻¹ * M * x = M :=
sorry

theorem exercise_2.4.8 {G : Type*} [group G] 
    (H : subgroup G) (h : ∀ (a : G), H * a = a * H) :
    ∀ (a : G), a * H * a⁻¹ = H :=
sorry

theorem exercise_2.4.26 {G : Type*} [group G] 
...
theorem exercise_2.4.32 {G : Type*} [group G] [fintype G]
    (H : subgroup G) (a : G) :
    ∃ (m : ℕ), a ^ m ∈ H ∧ nat.dvd (order a) m :=
sorry

theorem exercise_2.4.36 (a n : ℕ) (h : a > 1) :
    n ∣ nat.euler_phi (a ^ n - 1) :=
sorry

theorem exercise_2.5.6 {G : Type*} [group G] {G' : Type*} [group G']
    (f : G → G') (hf : is_group_hom f) : is_subgroup (set.range f) :=
sorry

theorem exercise_2.5.23 {G : Type*} [group G] 
    (hG : ∀ (H : subgroup G), H ≤ normalizer G H) (a b : G) :
    ∃ (j : ℕ), b * a = a ^ j * b :=
sorry

theorem exercise_2.5.30 {G : Type*} [group G] 
    (H : subgroup G) (hH : H.order = p) (hHn : H.normal) 
    (hG : G.order = p * m) (hp : p ∣ m) :
    H.is_char :=
sorry

theorem exercise_2.5.31 {G : Type*} [group G] 
    (p : ℕ) (h : nat.prime p) (n : ℕ) (m : ℕ) (hG : G.card = p ^ n * m) 
    (H : subgroup G) (hH : H.card = p ^ n) :
    characteristic G H :=
sorry

theorem exercise_2.5.37 (G : Type*) [group G] 
    (hG : G.card = 6) (hG' : ¬ abelian G) :
    G ≅ symmetric_group 3 :=
sorry

theorem exercise_2.5.43 (G : Type*) [group G] (hG : order G = 9) :
    abelian G :=
sorry

theorem exercise_2.5.44 {G : Type*} 
    [group G] (p : ℕ) (hp : nat.prime p) (hG : (p ^ 2 : ℕ) ∣ G.card) :
    ∃ (N : subgroup G), N.card = p ∧ N.normal :=
sorry

theorem exercise_2.5.52 {G : Type*} [group G] 
    (φ : G → G) (hφ : is_automorphism φ) (h : ∀ x : G, φ x = x⁻¹) : 
    is_abelian G :=
sorry

theorem exercise_2.6.15 {G : Type*} [group G] 
...
theorem exercise_2.7.7 {G : Type*} [group G] {G' : Type*} [group G']
    (φ : G →* G') (hφ : function.surjective φ) (N : set G) 
    (hN : is_normal_subgroup N) : is_normal_subgroup (φ '' N) :=
sorry

theorem exercise_2.8.7 {G : Type*} [group G] 
...
theorem exercise_2.9.2 {G₁ G₂ : Type*} [group G₁] [group G₂]
...
theorem exercise_3.3.2 {k : ℕ} (hk : k > 0) (σ : perm k) 
    (hσ : is_k_cycle σ k) :
    is_odd σ ↔ k % 2 = 0 :=
sorry

theorem exercise_3.3.9 (n : ℕ) (h : n ≥ 5) 
...
theorem exercise_4.1.28 {R : Type*} [ring R] [decidable_eq R] 
    [fintype R] [decidable_eq (matrix R 2 2)] :
    group (matrix R 2 2) :=
sorry

theorem exercise_4.1.29 {R : Type*} [comm_ring R] 
    (x : R) :
    det x = 0 ↔ x ≠ 0 ∧ x ∈ zero_divisors R :=
sorry

theorem exercise_4.1.34 (T : Type*) [group T] 
    (hT : ∀ (A : T), det A ≠ 0) :
    is_isomorphic T (perm.group 3) :=
sorry

theorem exercise_4.2.5 {R : Type*} [ring R] 
    (h : ∀ x : R, x ^ 3 = x) : comm_ring R :=
sorry

theorem exercise_4.2.6 {R : Type*} [ring R] (a x : R) 
    (h : a ^ 2 = 0) : a * (x + x * a) = (x + x * a) * a :=
sorry

theorem exercise_4.2.9 (p : ℕ) 
    (hp : nat.prime p) (hp1 : p % 2 = 1) :
    ∀ (a b : ℕ), a / b = ∑ i in (finset.range (p - 1) : finset ℕ), 1 / (i + 1) → p ∣ a :=
sorry

theorem exercise_4.3.1 {R : Type*} [comm_ring R] (a : R) :
    ideal.left_mul a = {x | x * a = 0} :=
sorry

theorem exercise_4.3.4 {R : Type*} [comm_ring R] (I J : ideal R) :
    ideal.is_ideal (I + J) :=
sorry

theorem exercise_4.3.25 {R : Type*} [ring R] 
    (M : Type*) [add_comm_group M] [module R M] [fintype M] [decidable_eq M] 
    (I : ideal R) :
    I = ⊥ ∨ I = ⊤ :=
sorry

theorem exercise_4.4.9 (p : ℕ) (hp : nat.prime p) :
    (p - 1) / 2 = card {x : ℕ | x < p ∧ x.quadratic_residue p} ∧
    (p - 1) / 2 = card {x : ℕ | x < p ∧ x.quadratic_nonresidue p} :=
sorry

theorem exercise_4.5.12 {F K : Type*} [field F] [field K]
    (hF : F ⊆ K) (f g : polynomial F) (hfg : f.coprime g) :
    f.map hF.to_embedding.to_fun.coprime g.map hF.to_embedding.to_fun :=
sorry

theorem exercise_4.5.16 {p : ℕ} (hp : nat.prime p) 
...
theorem exercise_5.5.2 : irreducible (polynomial.C (-1) * X^3 + polynomial.C 3 * X + 1) :=
sorry

theorem exercise_5.6.3 (p : polynomial ℚ) 
    (hp : p = X^4 + X^3 + X^2 + X + 1) :
    ∃ (K : Type*) [field K] [algebra ℚ K] (hK : degree K = 4), 
    is_splitting_field ℚ p K :=
sorry

theorem exercise_5.6.14 {F : Type*} [field F] 
    (p : ℕ) (hp : p ≠ 0) (n : ℕ) :
    (∀ (x : F), x ^ p ^ n - x = 0) → 
