theorem exercise_1.2 : is_cube_root_of_1 ⟨-1, 1⟩ :=
sorry

theorem exercise_1.3 (v : V) : -(-v) = v :=
sorry

theorem exercise_1.4 {F : Type*} [field F] {V : Type*} 
    [add_comm_group V] [vector_space F V] (a : F) (v : V) :
    a • v = 0 ↔ a = 0 ∨ v = 0 :=
sorry

theorem exercise_1.6 {R : Type*} [ring R] {U : set (R × R)} 
    (hU : U ≠ ∅) (hU_add : ∀ (u v : R × R), u ∈ U → v ∈ U → u + v ∈ U) 
    (hU_neg : ∀ (u : R × R), u ∈ U → -u ∈ U) :
    ¬ is_submodule U :=
sorry

theorem exercise_1.7 (U : set ℝ^2) (hU : U ≠ ∅) (hU_scalar : ∀ (a : ℝ) (x : ℝ^2), x ∈ U → a • x ∈ U) :
  ¬ is_subspace U :=
sorry

theorem exercise_1.8 {R : Type*} [comm_ring R] {I : Type*} 
    [decidable_eq I] {V : Type*} [add_comm_group V] [module R V] 
    (S : I → submodule R V) :
    is_submodule R (⋂ i, S i) :=
sorry

theorem exercise_1.9 {V : Type*} [add_comm_group V] 
    [vector_space ℂ V] (U W : submodule ℂ V) :
    submodule.is_subspace (U ⊔ W) ↔ U ≤ W ∨ W ≤ U :=
sorry

theorem exercise_2.1 {V : Type*} [add_comm_group V] [vector_space ℂ V]
...
theorem exercise_3.1 {V : Type*} [field ℂ] [add_comm_group V] 
    [vector_space ℂ V] {T : V → V} (hT : linear_map ℂ V V T) 
    (hdim : dim ℂ V = 1) :
    ∃ (a : ℂ), ∀ (v : V), T v = a • v :=
sorry

theorem exercise_3.8 {V : Type*} 
...
theorem exercise_7.17 {V : Type*} [inner_product_space ℂ V] 
    {A B : V →ₗ[ℂ] V} (hA : A.is_self_adjoint) (hB : B.is_self_adjoint) 
    (hApos : A.is_positive) (hBpos : B.is_positive) :
    (A + B).is_positive :=
sorry

theorem exercise_7.18 {V : Type*} [inner_product_space ℂ V] [normed_group V] 
