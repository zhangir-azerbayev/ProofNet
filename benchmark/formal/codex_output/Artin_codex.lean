theorem exercise_2.2.9 {G : Type*} [group G] {a b : G} 
    (h : a * b = b * a) : is_abelian (subgroup.generated G (a, b)) :=
sorry

theorem exercise_2.3.1 {R : Type*} 
    [ring R] (f : R →+* R) (hf : ∀ x : R, 0 < f x) :
    is_mul_group_hom f :=
sorry

theorem exercise_2.3.2 {G : Type*} [group G] (a b : G) :
    a * b ≈ b * a :=
sorry

theorem exercise_2.4.19 {G : Type*} 
    [group G] (hG : ∃ (x : G), x ≠ 1 ∧ order x = 2) :
    ∃ (x : G), x ≠ 1 ∧ order x = 2 ∧ x ∈ center G :=
sorry

theorem exercise_2.8.6 {G H : Type*} [group G] [group H] :
...
theorem exercise_2.11.3 {G : Type*} [group G] 
    (hG : even (card G)) : ∃ (g : G), g ≠ 1 ∧ g^2 = 1 :=
sorry

theorem exercise_3.2.7 {F : Type*} [field F] {G : Type*} [field G]
    (f : F → G) (hf : is_ring_hom f) : injective f :=
sorry

theorem exercise_3.5.6 {V : Type*} [add_comm_group V] [vector_space ℂ V]
    (hV : ∃ (s : set V), countable s ∧ span ℂ s = ⊤) :
    ∃ (s : set V), countable s ∧ is_basis ℂ s :=
sorry

theorem exercise_3.7.2 {V : Type*} [vector_space ℂ V] 
    (hV : ∀ (S : set V), finite S → ∃ (v : V), v ∉ ⋃₀ S) :
    ∀ (S : set V), finite S → ∃ (v : V), v ∉ ⋃₀ S :=
sorry

theorem exercise_6.1.14 (G : Type*) [group G] 
    (Z : set G) (hZ : is_subgroup Z) (hZc : is_center G Z) 
    (hGZ : cyclic (quotient_group.quotient Z)) :
    abelian G :=
sorry

theorem exercise_6.4.2 {p q : ℕ} (hp : nat.prime p) 
    (hq : nat.prime q) (h : p * q ≠ 1) :
    ¬ simple_group (multiplicative (fin (p * q))) :=
sorry

theorem exercise_6.4.3 {p q : ℕ} (hp : nat.prime p) 
    (hq : nat.prime q) (h : p ^ 2 * q ≠ 1) :
    ¬ simple_group (p ^ 2 * q) :=
sorry

theorem exercise_6.4.12 (G : Type*) [group G] (hG : card G = 224) :
  ¬ simple_group G :=
sorry

theorem exercise_6.8.1 {G : Type*} [group G] 
    (a b : G) :
...
theorem exercise_6.8.6 {G : Type*} [group G] 
    (N : subgroup G) (hN : N.normal) (hG : is_cyclic G) 
    (hGN : is_cyclic (G / N)) :
    ∃ (g h : G), G = ⟨g, h⟩ :=
sorry

theorem exercise_10.1.13 {R : Type*} [ring R] (x : R) (hx : ∃ n : ℕ, x ^ n = 0) :
    is_unit (1 + x) :=
sorry

theorem exercise_10.2.4 (x : polynomial ℤ) : 
    (ideal.span {2}).inter (ideal.span {x}) = ideal.span {2 * x} :=
sorry

theorem exercise_10.6.7 {I : ideal ℤ[i]} 
    (hI : I ≠ ⊥) : ∃ (z : ℤ), z ≠ 0 ∧ z ∈ I :=
sorry

theorem exercise_10.6.16 (f : polynomial ℤ) (a : ℤ) :
  ∃ (g : polynomial ℤ), f = g.eval₂ (λ x y, x - y) a :=
sorry

theorem exercise_10.3.24a {R : Type*} [comm_ring R] (I J : ideal R) :
    ¬ (I.is_ideal ∧ J.is_ideal) → ¬ (I.is_ideal ∧ J.is_ideal ∧ (I ∪ J).is_ideal) :=
sorry

theorem exercise_10.4.6 {R : Type*} [comm_ring R] 
    (I J : ideal R) (x : R) (hx : x ∈ I ∩ J) :
  is_nilpotent (x + I J) :=
sorry

theorem exercise_10.4.7a {R : Type*} [comm_ring R] 
    (I J : ideal R) (hIJ : I + J = ⊤) : I * J = I ∩ J :=
sorry

theorem exercise_10.5.16 {R : Type*} 
...
theorem exercise_11.3.4 {R : Type*} [integral_domain R] 
    [decidable_eq R] (f g : polynomial R) :
    is_rel_prime f g ↔ ∃ (a : R), a ∈ ideal.span {f, g} :=
sorry

theorem exercise_11.4.1b : irreducible (polynomial.C 12 + 6 * X + X ^ 3) :=
sorry

theorem exercise_11.4.6a : irreducible (polynomial.C (2 : ℤ)) (X^2 + X + 1) :=
sorry

theorem exercise_11.4.6b (x : ℤ/7) : irreducible (x^2 + 1) :=
sorry

theorem exercise_11.4.6c : irreducible (X^3 - 9 : polynomial ℤ_31) :=
sorry

theorem exercise_11.4.8 {p : ℕ} (hp : nat.prime p) (n : ℕ) :
    irreducible (polynomial.X ^ n - C p) :=
sorry

theorem exercise_11.4.10 irreducible_of_degree_two_n_plus_one_of_nonzero_leading_coeff_of_zero_coeffs_of_nonzero_constant_coeff_of_zero_coeffs_of_zero_coeffs_of_nonzero_constant_coeff_of_zero_coeffs_of_zero_coeffs_of_nonzero_constant_coeff_of_zero_coeffs_of_zero_coeffs_of_nonzero_constant_coeff_of_zero_coeffs_of_zero_coeffs_of_nonzero_constant :=
sorry

theorem exercise_11.9.4 {R : Type*} 
...
theorem exercise_13.6.10 {K : Type*} [field K] [fintype K] 
    (hK : ¬ (∃ (x : K), x ≠ 0 ∧ x ≠ 1)) : 
    ∏ (x : K), x ≠ 0 → x = -1 :=
sorry