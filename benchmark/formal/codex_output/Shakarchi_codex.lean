theorem exercise_1.13a {f : ℂ → ℂ} (hf : holomorphic f) 
    (hc : is_const (λ z, (f z).re)) : is_const f :=
sorry

theorem exercise_1.13b {f : ℂ → ℂ} 
    (hf : holomorphic f) (hc : ∀ z, f z ∈ set.Icc 0 0) :
    ∃ (c : ℂ), f = function.const ℂ c :=
sorry

theorem exercise_1.13c {f : ℂ → ℂ} (hf : holomorphic f) 
...
theorem exercise_1.19a (z : ℂ) (hz : abs z = 1) :
    ¬ (∃ (r : ℝ), abs (∑ n, n * z ^ n) < r) :=
sorry

theorem exercise_1.19b (z : ℂ) : 
    abs z = 1 → power_series.converges_at (λ n, z / n ^ 2) z :=
sorry

theorem exercise_1.19c (z : ℂ) (hz : abs z = 1) 
    (h1 : z ≠ 1) :
...
theorem exercise_1.26 {E : Type u} [normed_group E]
...
theorem exercise_2.5 {f : ℂ → ℂ} {Ω : set ℂ}
...
theorem exercise_2.6 {Ω : Type*} [complex_normed_vector_space Ω]
...
theorem exercise_3.3 (a : ℝ) (ha : 0 < a) :
    ∫⁻ a⁺ (λ x, cos x / (x ^ 2 + a ^ 2)) = π * (exp (-a) / a) :=
sorry

theorem exercise_3.4 (a : ℝ) (ha : 0 < a) :
    ∫⁻∞ ∞ (λ x, x * sin x / (x ^ 2 + a ^ 2)) = π * exp (-a) :=
sorry

theorem exercise_3.9 : ∫ (0 : ℝ) 1 (λ x, log (sin (π * x))) = -log 2 :=
sorry

theorem exercise_3.14 {f : ℂ → ℂ} (hf : entire f) 
    (hf_inj : function.injective f) :
    ∃ (a b : ℂ), f = λ z, a * z + b ∧ a ≠ 0 :=
sorry

theorem exercise_3.22 (D : set ℂ) 
...
theorem exercise_4.4a {α : Type*} [comm_ring α] 
...
theorem exercise_5.3 (α : ℝ) (hα : 0 < α) :
