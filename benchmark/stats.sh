for f in benchmark_to_publish/informal/*tex; do
    n=`grep '\paragraph{Exercise ' $f | wc -l`
    n_proofs=`grep 'begin{proof}' $f | wc -l`
    echo "$f: $n statements, $n_proofs proof"
done
echo
for f in benchmark_to_publish/formal/*lean; do
    n=`grep 'theorem exercise_' $f | wc -l`
    echo $f: $n
done
echo
N_INFORMAL=`grep -r '\paragraph{Exercise ' benchmark_to_publish/informal/*tex | wc -l`
N_PROOFS=`grep -r 'begin{proof}' benchmark_to_publish/informal/*tex | wc -l`
N_FORMAL=`grep -r theorem benchmark_to_publish/formal/*lean | wc -l`
