theorem exercise_1.27 (n : ℕ) (h : nat.odd n) : 
    8 ∣ n ^ 2 - 1 :=
sorry

theorem exercise_1.30 (n : ℕ) : 
    ¬ (∃ (k : ℤ), ∀ (m : ℕ), m < n → (1 : ℚ) / m = k) :=
sorry

theorem exercise_1.31 {a b : ℤ} (h : a ∣ b) (h' : a ∣ b) :
  ∃ (c : ℤ), a * c = b :=
sorry

theorem exercise_2.4 (a : ℕ) (h : a ≠ 0) (n m : ℕ) 
    (hnm : n > m) :
    nat.gcd (a ^ (2 ^ n) + 1) (a ^ (2 ^ m) + 1) = 1 ∨ nat.gcd (a ^ (2 ^ n) + 1) (a ^ (2 ^ m) + 1) = 2 :=
sorry

theorem exercise_2.21 (n : ℕ) :
...
theorem exercise_5.2 {p : ℕ} (hp : nat.prime p) (a : ℕ) 
    (ha : a % p ≠ 0) :
...
theorem exercise_12.30 (p q : ℕ) (hp : nat.prime p) 
